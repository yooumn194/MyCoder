...
There is one more boundary that outward-facing tools need: SSRF protection. The current `fetch_url` resolves every DNS answer and rejects loopback, private, link-local, reserved, and cloud-metadata destinations. It repeats that validation for every redirect hop and disables ambient proxies so another network boundary cannot silently resolve the target instead. Trusted local development hosts require an explicit `MYCODER_FETCH_ALLOWED_HOSTS` allowlist. Every time you add a tool that can take an outward action, first ask "what is the worst this could be used for?"

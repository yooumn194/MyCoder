...
这里还必须防一个常被忽略的风险：SSRF。当前 `fetch_url` 会解析目标主机的全部 DNS 结果，拒绝 loopback、私网、链路本地、保留地址和云元数据地址；重定向的每一跳也会重新校验，并禁用环境代理，避免把解析责任交给另一个网络边界。可信的本地开发地址只能通过 `MYCODER_FETCH_ALLOWED_HOSTS` 显式放行。每加一个能对外发起动作的工具，都先问一句「它最坏能被用来干嘛」。

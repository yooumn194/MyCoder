...
CORECODER_RATE_LIMIT=requests-per-minute on /v1/agent/run).
"""
...
class RateLimiter:
    ...
    def from_env(cls) -> "RateLimiter | None":
        """CORECODER_RATE_LIMIT=<requests/min> -> limiter, or None when unset."""
        import os

        raw = os.getenv("CORECODER_RATE_LIMIT", "").strip()
        if not raw:
            return None
        try:
            return cls(int(raw))

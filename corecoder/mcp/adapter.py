"""MCPToolAdapter: bridge one MCP server tool onto the CoreCoder Tool contract.
...
class MCPToolAdapter(Tool):
    ...
    def execute(self, **kwargs) -> str:
        ...
        call must happen there too. Errors propagate as MCPToolError
        (structured, classifyable) rather than being swallowed into a string.
        """
    ...
    def _convert_schema(mcp_schema: dict) -> dict:
        """MCP inputSchema (JSON Schema) -> CoreCoder Tool parameters.

        Both are JSON-schema flavoured; CoreCoder expects
        ...
        CoreCoder's Tool.parameters also uses).
        """

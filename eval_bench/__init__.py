"""MyCoder end-to-end evaluation harness (black-box over the HTTP API).
  ...
  * dataset.json — 30 hand-written coding problems (bugfix/refactor/implement/
                   ...
                   verification.
  * runner.py    — executes the dataset against a running MyCoder API:
                   POST /v1/agent/run -> poll /v1/agent/status -> run pytest.
  * scorer.py    — Pass@1 statistics + summary.json / report.md (+ optional
                   matplotlib chart).

The harness is fully independent: it never imports or modifies the mycoder

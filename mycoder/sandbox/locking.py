...
class AsyncMutex:
    ...
    The agent runs tools on a thread pool (mycoder/agent.py) and each tool
    ...
    @property
    def underlying(self) -> threading.Lock:
        """The raw threading.Lock, for synchronous callers (atexit cleanup)."""
        return self._lock

...
context. Plans persist to ~/.mycoder/plans/plan.json so a crashed run can be
resumed.
...
PLAN_DIR_ENV = "MYCODER_PLAN_DIR"
ENFORCE_PLANNING_ENV = "MYCODER_ENFORCE_PLANNING"
_TRUE = {"1", "true", "yes", "on"}
...
class PlanStore:
    ...
    def __init__(self, base_dir: Path | str | None = None) -> None:
        base = Path(base_dir or os.getenv(PLAN_DIR_ENV, "~/.mycoder/plans"))
...
# Read-only MCP tool name suffixes (mcp_{server}_{tool}): LSP definition /
# references / hover etc. are lookups, not mutations — they must NOT obey
# mutation plan discipline, or plan execution blocks every LSP query mid-plan
# (#15).
_READONLY_MCP_SUFFIXES = frozenset({
    "definition", "references", "hover", "get", "list", "search",
    "read", "status", "info", "inspect", "outline", "symbols",
})
...
def planning_guard(tool_name: str, query: str | None = None) -> str | None:
    ...
    Soft/hard semantics (optimization point #1):
      ...
      * with NO active plan: allowed by default (preserves Phase 1/2 open-ended
        behaviour), hard-blocked only when MYCODER_ENFORCE_PLANNING=1;
    ...
    if tool_name.startswith("mcp_"):
        # mcp_* tools obey plan discipline EXCEPT read-only queries —
        # mcp_lsp_definition / _references / _hover are lookups, not mutations,
        # and must not be blocked mid-plan (#15).
        if tool_name.rsplit("_", 1)[-1] in _READONLY_MCP_SUFFIXES:
            return None
    ...
    if plan is None:
        ...
        # 切换条件：当用户/系统设置 MYCODER_ENFORCE_PLANNING=1 时，无计划 +

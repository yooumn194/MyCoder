...
max_tokens_per_session defaults from Config (env MYCODER_MAX_TOKENS, 4096).
"""
...
logger = get_logger("mycoder.budget")

WARN_RATIO = 0.8

# A session budget is a CUMULATIVE cap across an agent run — distinct from
# Config.max_tokens (a per-response output cap). The old fallback to
# Config.max_tokens (4096) silently killed any API run that didn't pass
# max_tokens (#18). Default 100k tokens/session; MYCODER_SESSION_BUDGET to tune.
_DEFAULT_SESSION_BUDGET = 100_000
...
class TokenBudgetGuard:
    def __init__(
        ...
        self.max_tokens_per_session = (
            ...
            else int(os.getenv("MYCODER_SESSION_BUDGET", str(_DEFAULT_SESSION_BUDGET)))
        )
        self._tracer = tracer

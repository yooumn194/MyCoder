...
class LLMTracer:
    ...
    def list_sessions(self) -> list[str]:
        ...
        with self._lock:
            return sorted({t.session_id for t in self._traces})
    ...
    def _summarize_traces(self, traces: list, label: str) -> dict:
        ...
        p95_ttft = (
            sorted(ttfts)[min(len(ttfts) - 1, int(len(ttfts) * 0.95) - 1)]
            if ttfts
            else 0.0
        )

...
Two databases with identical schema:
  ...
  * project -> {project_root}/.mycoder/memory.db   (scope='project')
  * global  -> ~/.mycoder/memory.db                (scope='global')
...
class _Vec0VectorBackend(_VectorBackend):
    ...
    def _ensure(self, conn: sqlite3.Connection) -> None:
        ...
        try:
            ...
            try:
                # Python 3.12 disables SQLite extension loading by default; the
                # enablement is PER-CONNECTION, so it cannot be cached on the
                # backend instance (a probe conn in _make_vector_backend would
                # otherwise "succeed" while real conns never load the extension).
                conn.enable_load_extension(True)
            except (AttributeError, sqlite3.OperationalError):
                pass  # older / already-enabled builds
            sqlite_vec.load(conn)
            self._available = True
        except Exception:
            self._available = False
            raise RuntimeError("sqlite-vec extension unavailable") from None
...
class MemoryStore:
    def __init__(
        ...
        the real ~/.mycoder untouched."""
        project_root = Path(project_dir or os.getcwd()).expanduser().resolve()
        global_root = Path(
            global_dir or (Path.home() / ".mycoder")
        ).expanduser().resolve()
        self.embedder = embedder
        self.filter_sensitive = filter_sensitive
        self.project_db_path = project_root / ".mycoder" / "memory.db"
